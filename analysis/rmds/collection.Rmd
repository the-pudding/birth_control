...
#codeURL_0610 <- "ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/NSFG/sas/2006_2010_FemRespSetup.sas"
...
firstTimeYear <- allData %>% 
  ...
  mutate(decade = case_when(
    WNFSTUSE_Y < 1980 ~ "1970's",
    between(WNFSTUSE_Y, 1980, 1989) ~ "1980's",
    between(WNFSTUSE_Y, 1990, 1999) ~ "1990's",
    between(WNFSTUSE_Y, 2000, 2009) ~ "2000's",
    WNFSTUSE_Y >= 2010 ~ "2010's"
  )) %>% 
  group_by(decade) %>% 
  mutate(total = n()) %>% 
...
firstTimeYearPlot <- ggplot(firstTimeYear, aes(x = decade, y = percent, group = as.factor(FIRSMETH1), color = as.factor(FIRSMETH1))) +
  geom_line()
...
```{r}
possibleTypes <- c("CONDOM", "VASECTMY", "DEPOPROV", "WIDRAWAL", "RHYTHM", "SDAYCBDS", "TEMPSAFE", "PATCH", "PILL", "RING", "MORNPILL", "EVIUDTYP1", "EVIUDTYP2")
...
usageMap <- map_dfc(monthNumbers, condenseAll) %>% 
  mutate_all(funs(str_replace_all(., ",NA", ""))) %>% 
  cbind(idsOnly) %>% 
...
```
Alright, so there are just over 600 combinations. 

Let's start with the single usage methods. How long are they typically used? 
```{r}
...
ggplot(commonCombo, aes(m_a, m_b)) +
  geom_tile(aes(fill = log(unique)), color = "white") +
